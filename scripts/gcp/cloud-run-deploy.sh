...
case "$SERVICE" in
  api)
    PORT=4000
    WANT_SECRETS=(DATABASE_URL AUTH0_DOMAIN AUTH0_AUDIENCE REDIS_URL OPENSEARCH_URL OPENSEARCH_USERNAME OPENSEARCH_PASSWORD ADMIN_JWT_SECRET ADMIN_BOOTSTRAP_PASSWORD)
  ...
  web)
    ...
    ENV_VARS="NODE_ENV=production,PORT=3000,APP_BASE_URL=${APP_BASE_URL},NEXT_PUBLIC_APP_URL=${APP_BASE_URL},API_URL=${API_URL},NEXT_PUBLIC_ADSENSE_CLIENT=${NEXT_PUBLIC_ADSENSE_CLIENT:-ca-pub-6274053387170397}"
  ...
  admin)
    ...
    API_URL="${API_URL:-https://api.varnarc.com/api/v1}"
    ENV_VARS="NODE_ENV=production,PORT=3001,APP_BASE_URL=${APP_BASE_URL},API_URL=${API_URL},NEXT_PUBLIC_API_URL=${API_URL}"
