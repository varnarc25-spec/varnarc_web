...
type VersionDetail = PageVersionRow & {
  content: string;
};
...
export function PageVersionHistory({
  ...
  const fetchVersion = useCallback(
    async (versionId: string): Promise<VersionDetail> => {
      ...
      const res = await fetch(
        `/api/admin/cms/pages/versions/detail?pageId=${pageId}&versionId=${versionId}`,
      );
    ...
    [pageId, currentContent, currentTitle],
  );
  ...
  async function restore(versionId: string) {
    ...
    setMessage(null);
    try {
      const res = await fetch('/api/admin/cms/pages/versions/restore', {
        method: 'POST',
