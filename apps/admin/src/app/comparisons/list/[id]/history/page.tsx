...
export default async function ComparisonHistoryPage({
  ...
  const result = await apiServerFetch<HistoryRow[]>(`/comparisons/admin/${id}/history`);
  const rows = Array.isArray(result.data) ? result.data : [];

  return (
    <div>
      <PageHeader title="Comparison history" description="Audit log entries for this comparison." />
      ...
      <p className="mt-4 text-sm text-[var(--varnarc-subtle)]">
        <Link href="/comparisons/list" className="text-[var(--varnarc-brand)] hover:underline">
          Back to comparisons
        </Link>
      </p>
    </div>
