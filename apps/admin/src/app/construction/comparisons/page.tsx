...
import { ConstructionComparisonForm } from '@/components/construction-forms';
...
export default async function ConstructionComparisonsAdminPage() {
  const result = await apiServerFetch<ComparisonRow[]>('/construction/admin/comparisons?limit=50');
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Saved construction material comparisons for public compare pages."
        actions={<Badge>{rows.length} saved</Badge>}
      />

      <ConstructionComparisonForm />
      ...
      {result.error ? (
        ...
        <div className="overflow-x-auto rounded-lg border border-[var(--varnarc-border)] bg-[var(--varnarc-surface)]">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {rows.map((row) => {
                const ids = row.materialIds ?? row.ids ?? [];
                ...
                return (
                  <tr key={row.id} className="border-b border-[var(--varnarc-border)]">
                    ...
                    <td className="px-4 py-3">
                      <Link
                        href={`/construction/compare?${qs.toString()}`}
