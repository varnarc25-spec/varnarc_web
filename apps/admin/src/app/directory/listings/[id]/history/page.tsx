...
export default async function DirectoryListingHistoryPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const result = await apiServerFetch<HistoryRow[]>(`/directory/listings/admin/${id}/history`);
  const rows = Array.isArray(result.data) ? result.data : [];

  return (
    <div>
      <PageHeader title="Listing history" description="Audit trail for this directory listing." />
      ...
      <p className="mt-4 text-sm">
        <Link href="/directory/listings" className="text-[var(--varnarc-brand)] hover:underline">
          Back to listings
        </Link>
      </p>
    </div>
