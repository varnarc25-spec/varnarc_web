...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return buildAutomobileMetadata({
      path: `/automobile/comparisons/${slug}`,
      title: `${data.title} — Vehicle Comparison | Varnarc`,
      description: `Compare ${data.vehicles?.length ?? 0} vehicles side by side on price, fuel, mileage and specs.`,
      entityType: 'automobile_comparison',
      entityId: data.id,
    });
  } catch {
...
export default async function AutomobileComparisonDetailPage({ params }: Props) {
  ...
  const path = `/automobile/comparisons/${slug}`;

  return (
    <>
      <AutomobileSeo
        breadcrumbs={automobileHubBreadcrumbs([
          { name: 'Comparisons', path: '/automobile/comparisons' },
      ...
      >
        {vehicles.length >= 2 ? (
                ...
                {[
                  {
                    label: 'Ex-showroom',
                    key: 'exShowroomPrice',
                    fmt: (v: unknown) => (v != null ? `₹${v}` : '—'),
                  },
                  {
                    label: 'On-road (est.)',
                    key: 'estimatedOnRoadPrice',
                    fmt: (v: unknown) => (v != null ? `₹${v}` : '—'),
                  },
          ...
          <EmptyState
            ...
            message="Not enough published vehicles in this comparison."
          />
        )}
      </ContentLayout>
    </>
